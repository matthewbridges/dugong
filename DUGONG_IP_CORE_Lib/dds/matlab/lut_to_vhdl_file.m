...
fprintf(fid, 'type lut_type is array (0 to 2 ** LUT_ADDR_WIDTH - 1) of signed(LUT_AMPL_WIDTH - 1 downto 0);\n');
fprintf(fid, 'constant sine : lut_type := (\n');

for i = 1:lut_size
    fprintf(fid, '\t\t%d => to_signed(%d, LUT_AMPL_WIDTH),\n', i - 1, SINE_LUT(i));
end

fprintf(fid, '\t\tothers => to_signed(0, LUT_AMPL_WIDTH)\n);\n');
